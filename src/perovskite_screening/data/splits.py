...
class SplitConfig:
    ...
    @classmethod
    def from_config(cls, config: dict[str, object]) -> "SplitConfig":
        splits = dict(config.get("splits", config))
        val_size = float(splits.get("val_size", 0.10))
        test_size = float(splits.get("test_size", 0.10))
        train_size = 1.0 - val_size - test_size
        if train_size <= 0:
            raise ValueError("train/val/test fractions must sum to less than 1")
        return cls(
            random_seed=int(splits.get("random_seed", config.get("random_seed", 42))),
...
def add_chemistry_keys(df: pd.DataFrame) -> pd.DataFrame:
    missing = {"sample_id", "structure", "target"} - set(df.columns)
    if missing:
        raise ValueError(f"Dataset is missing required columns: {sorted(missing)}")
    enriched = df.copy()
    enriched["formula"] = [structure.composition.reduced_formula for structure in enriched["structure"]]
...
def assignments_from_ids(
    *,
    ...
    for split, ids in (("train", train_ids), ("val", val_ids), ("test", test_ids)):
        rows.extend(
            {"sample_id": int(sample_id), "split": split, "split_strategy": split_strategy}
...
def build_element_set_split(df: pd.DataFrame, config: SplitConfig) -> pd.DataFrame:
    if "element_set_key" not in df.columns:
        df = add_chemistry_keys(df)
    group_sizes = (
    ...
    for row in group_sizes.itertuples(index=False):
        ...
        deficits = {split: target_sizes[split] - assigned_sizes[split] for split in SPLIT_NAMES}
    ...
    return assignment[["sample_id", "split", "split_strategy"]].astype({"sample_id": int})


def build_split(df: pd.DataFrame, strategy: str, config: SplitConfig) -> pd.DataFrame:
    if strategy == "random_iid":
        return build_random_iid_split(df, config)
    if strategy == "element_set":
        return build_element_set_split(df, config)
...
def validate_split_assignment(
    *,
    ...
    missing = {"sample_id", "split", "split_strategy"} - set(assignment.columns)
    if missing:
    ...
    if invalid_strategies:
        raise ValueError(f"{split_strategy} has invalid split_strategy values: {sorted(invalid_strategies)}")
...
def validate_element_set_no_leakage(df: pd.DataFrame, assignment: pd.DataFrame) -> None:
    if "element_set_key" not in df.columns:
        df = add_chemistry_keys(df)
...
def load_split(split_strategy: str, split_dir: Path | None = None) -> pd.DataFrame:
    base_dir = split_dir or project_path("data", "splits")
    frames = []
    for split in SPLIT_NAMES:
        path = base_dir / split_strategy / f"{split}.csv"
        if not path.exists():
        ...
        frames.append(frame)
    assignment = pd.concat(frames, ignore_index=True).astype({"sample_id": int})
    validate_split_assignment(
        assignment=assignment,
        all_sample_ids=assignment["sample_id"].astype(int).tolist(),
        split_strategy=split_strategy,
    )
    return assignment
...
def build_split_diagnostics(
    *,
    ...
    for split_strategy, assignment in assignments.items():
        ...
        split_frames = {split: enriched.loc[enriched["split"] == split].copy() for split in SPLIT_NAMES}
        formula_sets = {split: set(split_frames[split]["formula"]) for split in SPLIT_NAMES}
        element_set_sets = {split: set(split_frames[split]["element_set_key"]) for split in SPLIT_NAMES}
        ...
        for split, split_df in split_frames.items():
            ...
            for left, right in combinations(SPLIT_NAMES, 2):
                row[f"formula_overlap_{left}_{right}"] = int(len(formula_sets[left] & formula_sets[right]))
...
def load_split_metadata(split_strategy: str, split_dir: Path | None = None) -> dict[str, object]:
    base_dir = split_dir or project_path("data", "splits")
    return read_json(base_dir / split_strategy / "split_metadata.json")
